abun.heatmap.g <- function(ARG_type,group,Group_numb){
    ...
    colnames(ARG_type2) <- c("Gene","variable","value")
    ...
    for (j in 3:ncol(ARG_type5)) {
        ARG_type5[,j] <- ARG_type5[,j]/ARG_type5[,2]
    }
    ARG_type5 <- melt(ARG_type5)
    colnames(ARG_type5)[3] <- "value2"
    ARG_type5[ARG_type5$variable == levels(group$Group)[1],"value2"] <- 1
    colnames(ARG_type5)[2] <- "Group"
    ...
    p <- ggplot(ARG_type3,aes(Group,Gene)) +
        ...
        guides(fill = guide_colorbar(barheight = 5),colour = "none")
